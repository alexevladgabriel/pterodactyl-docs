...
When running Pterodactyl behind a reverse proxy, such as [Cloudflare's Flexible SSL](https://developers.cloudflare.com/ssl/origin-configuration/ssl-modes)
...
If you're using Cloudflare's Flexible SSL you should set `TRUSTED_PROXIES` to contain [their IP addresses](https://cloudflare.com/ips).
...
You can generate your own keys in the [reCAPTCHA Admin Console](https://google.com/recaptcha/admin).
...
Currently, all telemetry collection logic is handled by the [TelemetryCollectionService](https://github.com/pterodactyl/panel/blob/1.0-develop/app/Services/Telemetry/TelemetryCollectionService.php#L52) on the panel.
This service is responsible for collecting all the data that is sent to the remote
server.
