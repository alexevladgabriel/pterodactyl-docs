...
# MariaDB repository setup script can be skipped on Ubuntu 22.04
...
::: tip Database Configuration {icon-type=bordered}
...
please have a look at [Setting up MySQL](/tutorials/mysql_setup).
...
::: danger Back up your encryption key (APP_KEY in the `.env` file).
It is used as an encryption key for all data that needs to be stored securely (e.g. api keys). Store it somewhere safe - not just on your server. If you lose it all encrypted data is irrecoverable -- even if you have database backups.
:::
...
Finally, enable the Pterodactyl Queue service and set it to boot on machine start.
...
software is being used. To learn more about this feature and to opt-out, please see our [Telemetry](/panel/1.0/additional_configuration.md#telemetry)
