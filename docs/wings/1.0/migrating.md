...
[install guide](/wings/1.0/installing) if you are trying to install Wings for the first time on
a new node.
...
used anymore. To do this, simply execute the following commands &mdash; assuming your old daemon is in the default
...
If you've used the [standalone SFTP server](/daemon/0.6/standalone_sftp) with the old daemon, we need to remove it's systemd service as well, as it's no longer needed.
...
If you encounter issues starting Wings at this point, make sure Wings are stopped, then
run the following command to start Wings directly and check for any specific error output.
